...
export async function getAllKomoditasService() {
    const komoditas = await prisma.komoditas.findMany({ include: { jenis: { select: { name: true } } } })
    return komoditas
}

export async function getKomoditasByIdService(id: number) {
    const komoditas = await prisma.komoditas.findUnique({ where: { id } })
...
export async function createdKomoditasService
    (id_jenis: number, nama: string, deskripsi: string, foto: string, satuan: string, jumlah: number) {
...
export async function deleteKomoditasService(id: number) {
    ...
    const deleted = await prisma.komoditas.delete({ where: { id } })
    return deleted
}