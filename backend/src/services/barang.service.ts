...
export async function getAllBarangService() {
  const bawang = await prisma.barang.findMany({
    include: {
      TransaksiBarang: { orderBy: { tanggal: "asc" } }
    },
  });

  const barangs = bawang.map((barang) => {
    let jumlah = 0;
    barang.TransaksiBarang.forEach((transaksi) => {
      jumlah += transaksi.masuk - transaksi.keluar;
    });
    return { ...barang, jumlah: jumlah };
  });
